...
for (let index = 0; index < mahasiswa.length; index++) {
    if (mahasiswa[index].bachelorDegree) {
        bachelorDegree.push(
            {
...
for (let xiaomi = 0; xiaomi < postGraduate.length; xiaomi++) {
    console.log("nama : " + postGraduate[xiaomi].name + "\nKeahlian" + postGraduate[xiaomi].mastery);

}

console.log("mahasiswa S1");
for (let jack = 0; jack < bachelorDegree.length; jack++) {
    console.log("nama : " + bachelorDegree[jack].name + "\nIPK" + bachelorDegree[jack].GPA);
